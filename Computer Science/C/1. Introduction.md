...
- For many years the defacto version of C was the one provided with UNIX.
- 1983 -> a commitee was established to create an ANSI standard for C.

- In 1989, the ANSI C standard was finally adopted. Along with being adopted by ISO. The resulting standard was typically referred to as ANSI/ISO standard C.

- This standard along with Amendment 1 of 1995 formed the base document for C++. This standard defined in 1989 is commonly referred to as C89.

- However, C continued to be worked upon and in 1999, came about the 1999 standard for C refered to as C99.

	- The C99 standardization focused on two main areas:
		- The addition of seceral numeric libraries and development of some special use, but highly innovative new features such as variable length arrays and the `restrict` pointer qualifier.

## Features of C
- C is a middle-level language
	- because it combines the best elements of high-level languages with the control and flexibility of assembly language.
	- As a middle-level language, C allows the manipulaition of bits, bytes, words, pointers and addresses - the base elements with which a computer functions.
	- Despite this C is very portable.
	- Unlike most high level languages, C specifies almost no runtime error checking.
	- Another important aspect of C is that it has a low amount of keywords. C89 has only 32 keywords, and C99 adds just 5 more.
- C is a structured language.
	- The distinguishing feature of a structured language is the compartmentalization of code and data.
	The reason that C is **not a block-structured language** is that block structured languages permit proces or functions to be declared inside other procedures or functions, which C does not permit.
- C is primarily a compiled language.
	- Compilation is a one-time cost, while interpretations incurs an overhead each time the program is run.
	- This also makes the program highly portable.
- C is case-sensitive:
	- so `else` is a keyword while `Else` is not.
- C also supports seperate compilation:
	- Therefore C allows a program to be spread across two or more files and it lets you compile each file seperately.
		- Once all the files are compiled, they are linked along with any library routines to form the complete object code.
	- The advantage of seperate compilation is that if you change the code of one file, only that file (and not the whole program) needs to be recompiled.
...
- As a general rule, the only function that must be present is `main()`, which is the first function called when program execution begins.
- Although `main` is not a keywod, it is good practice to treat it as if it were.
```c
...
```
## C Compilation
C compilation consists of 3 steps:
...
- A compiled C prgram and uses 4 logically distinct regions of memory:
	...
	- The remaining two regions are the stack and the heap.
		- The stack is used for things such as storing return addresses of function calls, arguments to functions, local variables and current state of the CPU, etc.
		- The heap is a region of free memory that your program can use via C's dynamic memory allocation functions.
	- Although the exact physical layout of each of the four regions of memory differs among CPU types and C implementations, the conceptual layout is as follows:
	<img src="https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fmiro.medium.com%2Fv2%2Fresize%3Afit%3A1200%2F1*3M10Yuk0xd6kXSm2k1W77A.png&f=1&nofb=1&ipt=87884f25725a75ba33cd481e11f9ceb03fab2018ea7ac1a04cf7915e59137d2f">

### C Libraries & Linking
- Most predefined functions such as `printf()`, `sqrt()`, etc. are contained in C libraries.
- All C compilers come with a standard library of functions that perform most commonly needed tasks.
- Some compilers include a linker, while others used the standard linker provided by the operating system.
- Linking is the process where the linker combines the written code with the object code contained in the libraries.
- The functions in the library are in relocatable format. This means that memory addresses for the various machine-code instructions have not been absolutely defined - only offset information has been kept.
...
- There are 32 keywords in ANSI C:
	- C99 adds just 5 more.

`auto` `double` `int` `struct` `break` `do` `else` `long` `switch` `goto` `sizeof` `if` `case` `enum` `register` `typedef` `default` `char` `extern` `return` `union` `volatile` `const` `float` `short` `unsigned` `while` `continue` `for` `signed` `void` `static`
...
- Rules for identifier names:
	...
	-  Cannot use the same name as keywords or the same name as funcitons in the C library.
	-  Cannot have two successive underscores (`__`)
	-  Max length is 31 characters. <font color=red>//check later</font>

- C defines two kinds of identifiers: external & internal
	- An external identifier will be involved in an external link process. These identifiers, called external names, include funciton names and global variables names that are shared between source files.
	...
	- For example:
		- C89: atleast the first 6 characters of an external identifier and at least the first 31 characters of an internal identifier will be significant.
		- C99: external has at least 31 and interal has at least 63 significant characters.
		- C++: at least the first 1024 characters are significant.
...
- Variables can be declared in three okaces:
	- Inside functions (local variables)
	- In the defiinition of function parameters (formal parameters)
	- Outside of all functions (global variables)

- A local variable is created upon entry into its code block and destroyed upon exit.

- The keyword `auto` is used to declare gloval variables. However, since all nonglobal variables are by default, assumed to be `auto`, this keyword is virtually never used.

- When a variable declared within an inner block has the same name as a variable declared by an enclosing block, the inner variable *hides* the outer variable.

### Scope of a Variable
- Standard C defines 4 scopes that determine the visibility of an identifier.
	1) File Scope:
		- Starts at he beginning of the file (translation unit) and ends with the end of the file.
		- Refers to only those variables declared outside of functions (visible throughout the file).
		- Variable with file scope are global.
	2) Block Scope:
		- Lies within the brackets `{}` of a code block.
		- Block scope also extends to function paramters in a function definition.
	...
	4) Function Scope:
		- Lies within `{}` of a function.
		- Function scope applies only to labels.
		- The label must be within the same function as `goto`.
...
- In C89, all local variables must be declared at the start of the block, prior to any action statements.
	- for example:
	```c
	void func(){
		int i;
		i=10;
		int j; // will cause error in C89
		j=20; // This line would've been valid in C99/C++
	}
	```

- We can direct the compiler to retun the values of local variables between function calls by using the `static` keyword.

- Global variables are known througout the program and can be used by any piece of code. Also, they hold their value throughout the execution of the program.
	- They are created by declaring them outside of any function.
	- If a global variable and local variable have the same name, the local variable is always reference, if it is in scope, that is.
...
There are 5 fundamental data types in C89:
...
-The size and range of these datatypes may vary across processor types and compilers:
	- In all cases, `char` is 1 byte.
	- The size of `int` is usually the same as the word length of the execution enviroment of the program: dependent on 16/32/64-bit environments.
	- It is important to note that C stipulates only the minimal range of each data type, not its size in bytes or its typical range.

- Except `void`, the basic data types have various modifiers preceeding them:
	- `signed`
	- `unsigned`
	- `long`
	- `short`

- `char` can be used with `signed` or `unsigned`.
- `int` can be used with all modifiers.
- `double` can be used with `long`.

**Note:** `long` can be used with `int` twice in C99, creating `long long int`.

- All default declarations except `char` are `signed`, wheras `char` is `unsigned` by defualt.

- When a type modifier is used then `int` is assumed.
	- example: `long` becomes `long int`.

## Type qualifiers:
- C defines type qualifiers that control how vriables may be accessed or modified.
- C89 contains two: `const` and `volatile` while C99 adds a third: `restrict`.
